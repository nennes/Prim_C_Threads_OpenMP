	...
	*/
	...
	else
	{
		...
		*/
		...
		graphTable=malloc(nodeNumber * sizeof(*graphTable));
		...
		*/
		...
		graphTree=malloc(nodeNumber * sizeof(*graphTree));
		for (temp=0; temp<nodeNumber; temp++) 
		{
		...
		*/
		...
		allocSize = 3 * nodeNumber;
		/*
		We assume that each line will contain two-char node weights (max 99).
		Adding the space delimiter we allocate 3 * nodeNumber characters.
